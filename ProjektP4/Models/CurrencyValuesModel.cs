...
public class CurrencyValuesContext : DbContext
{
    ...
    public async void SyncCurrencyValues()
    {
        HttpRequestMessage req = new HttpRequestMessage(
            ...
            + Environment.GetEnvironmentVariable("FREECURRENCYAPI_API_KEY")
            + "&currencies=EUR%2CUSD&base_currency=PLN"
        );
    ...
    {
        modelBuilder.Entity<CurrencyValues>(
            cv =>
            {
                cv.HasKey(["currencyValueId"]);
            }
        );
    }
